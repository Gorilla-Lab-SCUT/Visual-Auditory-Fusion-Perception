...
```bash
python chatme.py --image asserts/demo.jpg --question "苹果在桌子的哪里？"
```

此命令会加载预训练的模型，并使用提供的图片(`demo.jpg`)和问题(`"苹果在桌子的哪里?"`)进行推理。
...
一旦您输入了问题(例如`"苹果在桌子的哪里?"`)，模型就会分析图片并尝试回答，推理结果将以文本形式输出到终端中，例如:
